  ...
  document.body.insertAdjacentHTML("beforeend", `
    ...
    <div style="background:#fff;border-bottom:1px solid #e2e8f0;position:sticky;top:58px;z-index:90">
      <div class="bk-container" style="padding-top:.65rem;padding-bottom:.65rem;display:flex;align-items:center;gap:.75rem;flex-wrap:wrap">
        ...
        <button id="bk-map-btn" onclick="BKI.toggleMap()" class="bk-btn bk-btn-outline" style="padding:.4rem .9rem;font-size:.8rem">
          🗺 Mapa
        </button>
      </div>
    </div>
    ...
    <footer class="bk-footer">
      ...
      <a href="../index.html">BeautyPolisa OC</a>
    </footer>
  `);

  document.getElementById("bk-search").addEventListener("keydown", e => {
    if (e.key === "Enter") BKI.doSearch();
  });
}
...
function renderCards(reset) {
  ...
  if (reset) {
    if (filtered.length === 0) {
      grid.innerHTML = `<div class="bk-empty" style="grid-column:1/-1">
        <h3>Brak wyników</h3>
        <p>Spróbuj innych słów kluczowych lub usuń filtry.</p>
      </div>`;
    } else {
      grid.innerHTML = filtered.slice(0, end).map(card).join("");
    }
  } else {
    filtered.slice(page * PER_PAGE, end).forEach(s =>
...
function card(s) {
  ...
  return `
    <a href="salon.html?id=${BK.esc(s.id)}" role="listitem"
       class="bk-card"
      ...
      <div style="height:190px;overflow:hidden;background:linear-gradient(135deg,#ede9fe,#ddd6fe);position:relative">
        ${cover
          ? `<img src="${BK.esc(cover)}" alt="${BK.esc(s.name)}" loading="lazy" style="width:100%;height:100%;object-fit:cover">`
...
window.BKI = {
  ...
  toggleMap() {
    showMap = !showMap;
    const mapEl = document.getElementById("bk-map");
    const btn   = document.getElementById("bk-map-btn");
    mapEl.style.display = showMap ? "block" : "none";
    btn.textContent = showMap ? "☰ Lista" : "🗺 Mapa";
    if (showMap) {
      refreshMap();
